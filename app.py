...
with tab1:
    ...
    # Veiligheidscheck: alleen delen als kosten > 0
...
with tab2:
    ...
    woorden = st.number_input("Aantal woorden", min_value=0, value=0)
    ...
    # Veiligheidscheck: alleen delen als inkoop > 0
...
with tab3:
    ...
    v_woord = st.number_input("Verkoopprijs per woord (€)", format="%.4f", value=0.0000)
    aantal = st.number_input("Aantal woorden", min_value=0, value=0, key="s3_aantal")
    target_markup = st.number_input("Gewenste marge (Markup) %", value=30.0)
    revisie_v_w = st.number_input("Inkoopprijs revisie per woord (€)", format="%.4f", value=0.0200, key="s3_rev")

    tot_verkoop = v_woord * aantal
    
    # Inkoop = Verkoop / (1 + Markup_decimal)
    max_totale_inkoop = tot_verkoop / (1 + (target_markup / 100))
    
    # Veiligheidscheck: alleen delen als aantal woorden > 0
    max_inkoop_p_w_incl = max_totale_inkoop / aantal if aantal > 0 else 0
    max_inkoop_p_w_excl = max_inkoop_p_w_incl - revisie_v_w
